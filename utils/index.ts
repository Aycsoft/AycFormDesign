...
export function makeTreeOptions(pre, config, level, data: any = []) {
  if (!config.id) {
    config.id = 1
  }
  level && level--
...
export function makeTreeOptionsRule(t, to, label = 'label', value = 'value') {
  const options = [
    { label: t('fetch.optionsType.struct'), value: 2 },
    { label: t('fetch.optionsType.fetch'), value: 1 }
  ]
...
const _toJSON = {
  ...
  array: function (val) {
    for (let i = 0, json: any = []; i < val.length; i++) {
...
export const translate = (path, option, locale) =>
  deepGet(locale, path, '').replace(/\{(\w+)\}/g, (_, key) => `${option?.[key] ?? `{${key}}`}`)
...
export function compareVersion(v1, v2) {
  ...
  for (let i = 0; i < minLength; i++) {
    const diff = parseInt(a1[i], 10) - parseInt(a2[i], 10)
    if (diff > 0) {
      return 1
    } else if (diff < 0) {
      return -1
