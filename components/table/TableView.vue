...
<template>
  <div class="_fd-table-view" :class="{ 'is-mini': mini }">
    ...
    <div class="_fd-table-context-menu" v-if="visible" :style="menuPos">
      <div class="_fd-table-context-menuitem" @click.stop="selectionStyle">{{
        t('props.style')
      }}</div>
      <div class="_fd-table-context-menuitem" @click.stop="rmSelectionContent">{{
        t('props.clear')
      }}</div>
...
export default defineComponent({
  ...
  methods: {
    ...
    rowResize(e, { pid, idx }) {
      ...
      if (
        !this.$refs[`td_${key}`] ||
        this.lattice[pid][idx].colspan > 1 ||
        this.lattice[pid][idx].rowspan > 1
      ) {
        key = `${pid}:${idx}`
      }
