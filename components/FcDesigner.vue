  ...
  >
    <el-main>
      <el-container style="height: 100%" :key="locale && locale.name">
        ...
        >
          <div class="_fc-l-close" @click="hiddenLeft = true"
            ><i class="fc-icon icon-arrow"></i
          ></div>
          <LanguageConfig v-if="activeModule === 'language'" />
          <JsonPreview v-if="activeModule === 'json'" />
              ...
              <template v-for="(item, index) in menuList">
                <div class="_fc-l-group" :key="index" v-if="hiddenMenu.indexOf(item.name) === -1">
                  <h4 class="_fc-l-title" @click="item.hidden = !item.hidden">
                    ...
                    <i class="fc-icon icon-arrow" :class="{ down: !item.hidden }"></i>
                  </h4>
                  <fcDraggable
              ...
              <template v-if="!inputForm.state">
                <template v-if="getConfig('showDevice') !== false">
                  <div class="devices">
                    ...
                    ></i> -->
                    <Icon
                      :icon="'vi-ant-design:tablet-outlined'"
                      :size="16"
                      class="fc-icon"
                    ...
                    ></i> -->
                    <Icon
                      :icon="'vi-ant-design:mobile-outlined'"
                      :size="16"
                      class="fc-icon"
            ...
            <div class="_fc-m-tools-r">
              <template v-if="!inputForm.state">
                ...
                <BaseButton
                  ...
                  @click="openPreview"
                >
                  <Icon class="fc-icon" :icon="'vi-ant-design:eye-outlined'" :size="14" />
                  {{ t('props.preview') }}
                </BaseButton>
                <el-popconfirm
                ...
                >
                  <template #reference>
                    <BaseButton type="danger" plain size="small">
                      <Icon class="fc-icon" :icon="'vi-ant-design:delete-outlined'" :size="14" />
                      {{ t('props.clear') }}
                    </BaseButton>
                  </template>
                </el-popconfirm>
                ...
                <div class="_fd-input-btn">
                  ...
                  >
                    <template #active-action>
                      <i class="fc-icon icon-edit2 unset" style="font-size: 12px; color: #fff"></i>
                    </template>
                    <template #inactive-action>
                      <i class="fc-icon icon-edit2 unset" style="font-size: 12px; color: #333"></i>
                    </template>
                  </el-switch>
                </div>
              </template>
        ...
        >
          <div class="_fc-r-close" @click="hiddenRight = true"
            ><i class="fc-icon icon-arrow"></i
          ></div>
          <el-container style="height: 100%">
            ...
            >
              ...
              >
                ...
                <TypeSelect :disabled="activePermission.switchType === false" />
                <template
                  v-if="
                    activePermission.name !== false &&
                    activeRule &&
                    activeRule.name &&
                    config.showComponentName !== false
                  "
                >
                  <p class="_fc-r-title">
        ...
        <div class="_fc-l-open" v-if="hiddenLeft" @click="hiddenLeft = false"
          ><i class="fc-icon icon-arrow"></i
        ></div>
        <div class="_fc-r-open" v-if="hiddenRight" @click="hiddenRight = false"
          ><i class="fc-icon icon-arrow"></i
        ></div>
        <el-dialog v-model="preview.state" width="80%" class="_fd-preview-dialog" append-to-body>
          <el-tabs class="_fd-preview-tabs" v-model="previewStatus">
            <el-tab-pane :label="t('form.formMode')" name="form" />
            <el-tab-pane :label="t('form.componentMode')" name="component" />
            <el-tab-pane :label="t('form.htmlMode')" name="html" />
          </el-tabs>
          <div
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const configFormOrderStyle = computed(() => {
      ...
      const sort = configRef.value.configFormOrder ? [...configRef.value.configFormOrder] : []
      let value = []
      if (!sort.length) {
        value = def
      } else {
    ...
    const methods = {
      ...
      setOption(opt) {
        ...
        options._submitBtn =
          typeof options.submitBtn === 'object'
            ? options.submitBtn
            : { show: options.submitBtn ? options.submitBtn !== false : false }
        // console.log(options.submitBtn)
      ...
      toolActive(rule) {
        if (
          configRef.value.beforeActiveRule &&
          false === configRef.value.beforeActiveRule({ rule })
        ) {
          return
        }
        methods.unWatchActiveRule()
        ...
        nextTick(() => {
          data.baseForm.api.all().forEach((item) => {
            if (item.name || item.field) {
              item.hidden =
                hiddenBaseField.indexOf(item.name) !== -1 ||
                hiddenBaseField.indexOf(item.field) !== -1
            }
          })
        ...
        if (data.validateForm.isShow) {
          nextTick(() => {
            data.validateForm.api.hidden(
              methods.getConfig('validateOnlyRequired') === true ||
                rule._menu.validate === 'required',
              ['validate']
            )
          })
        }
        ...
        ;(configRef.value?.componentPermission || []).forEach((item) => {
          ...
          if (
            ...
            (Array.isArray(item.field)
              ? item.field.indexOf(rule.field) > -1
              : item.field === rule.field)
          ) {
            flag = true
          } else if (
            item.name &&
            (Array.isArray(item.name) ? item.name.indexOf(rule.name) > -1 : item.name === rule.name)
          ) {
            flag = true
          } else if (
            item.id &&
            (Array.isArray(item.id) ? item.id.indexOf(rule._fc_id) > -1 : item.id === rule._fc_id)
          ) {
            flag = true
          } else if (
            item.tag &&
            (Array.isArray(item.tag)
              ? item.tag.indexOf(rule._fc_drag_tag) > -1
              : item.tag === rule._fc_drag_tag)
          ) {
            flag = true
          }
          if (flag) {
      ...
      checkDragToContainer(menu) {
        return (
          !menu.allowDragTo ||
          (Array.isArray(menu.allowDragTo)
            ? menu.allowDragTo.indexOf('draggable') > -1
            : menu.allowDragTo === 'draggable')
        )
      },
      dragPut(to, from, dragEl) {
        const toRule = methods.getTrueRule(to.el.__rule__)
        const _menu = dragEl._underlying_vm_.__fc__
      ...
      checkDrag({ menu, toMenu, rule, toRule }) {
        ...
        if (
          toRule.children &&
          toMenu.maxChildren &&
          toMenu.maxChildren <= toRule.children[0]?.children?.length
        ) {
          return false
        }
        if (
      ...
      checkAllowDrag(from, to) {
        ...
        if (
          from.allowDragTo &&
          (Array.isArray(from.allowDragTo)
            ? from.allowDragTo.indexOf(to.name) === -1
            : from.allowDragTo !== to.name)
        ) {
          return false
        }
      ...
      makeRule(config, _rule) {
        ...
        if (!_rule && updateRule) {
          if (typeof updateRule === 'function') {
            ...
            const _rule = deepCopy(updateRule)
      ...
      handleRemoveBefore({ rule }) {
        if (
          configRef.value.beforeRemoveRule &&
          false === configRef.value.beforeRemoveRule({ rule })
        ) {
          return false
        }
      },
      ...
      bindHotkey(event) {
        ...
        if (
          ...
          ['ArrowUp', 'ArrowDown', 'Backspace', 'c', 'x', 'z', 'p', '1', '2', '3', '4'].indexOf(
            event.key
          ) > -1
        ) {
