  ...
  <div class="_fc-signature" :class="{ disabled: disabled }">
    ...
    <template v-else>
      <div class="_fc-signature-btn" @click="open">
        <i class="fc-icon icon-edit2"></i>
        {{ formCreateInject.t('form.signaturePadTip') || '点击添加手写签名' }}
      </div>
    </template>
    <el-dialog
      class="_fc-signature-dialog"
      :title="formCreateInject.t('form.signaturePadTitle') || '请在虚线框内书写'"
      v-model="visible"
      destroy-on-close
  ...
  watch: {
    visible(val) {
      if (val) {
        this.isEmpty = true
        this.$nextTick(() => {
          this.signaturePad = markRaw(
            new SignaturePad(this.$refs.pad, {
...
._fc-signature-pad {
  ...
  background-image: linear-gradient(#ffffff 14px, transparent 0),
    linear-gradient(90deg, #ffffff 14px, #d4d7e0 0);
